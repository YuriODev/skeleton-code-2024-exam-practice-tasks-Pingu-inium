...
class Pattern():
  def __init__(self, SymbolToUse, PatternString, PatternAttempt):
    self.__Symbol = SymbolToUse
    self.__PatternSequence = PatternString
    ### CHANGE STARTS HERE ###
    self.__PatternCount = PatternAttempt

  def OutPutPatternCount(self):
    return f"Number of patterns left with the {self.__Symbol} symbol is {self.__PatternCount}."
  ...
  def MatchesPattern(self, PatternString, SymbolPlaced):
    if self.__PatternCount > 0:
      ...
      print(f"You cannot enter anymore {self.__Symbol} patterns.")
      return False
    ### CHANGE ENDS HERE ###

...
class Puzzle():
  ...
  def __SavePuzzle(self):
    name = str(input("Enter the name of the file you want to save the game to: "))
    ...
    for Pattern in self.__AllowedPatterns:
      file.write(Pattern._Pattern__Symbol + "," + Pattern._Pattern__PatternSequence + "\n")
    file.write(str(self.__GridSize) + "\n")
    for index in range(0, self.__GridSize * self.__GridSize):
      C = self.__Grid[index]
      items = []
      items.append(str(C._Symbol))
      for bannedsymbol in C._Cell__SymbolsNotAllowed:
        S = "," + str(bannedsymbol)
        items.append(S)
      if len(items) <= 1:
        items.append(",")
  ...
  def AttemptPuzzle(self):
    ...
    while not Finished:
      ...
      choice = input("Enter y if you want to save the game, enter n if you do not want to save: ")
      if choice == "y":
        self.__SavePuzzle()

...
class Puzzle():
  ...
  def AttemptPuzzle(self):
    ...
    while not Finished:
      ...
      ### CHANGE STARTS HERE ###
      print(f"Current number of symbols left: {self.__SymbolsLeft}")
      remove = input("Enter Y if you wish to remove a symbol, and enter N if you do not.")
      if remove == 'Y':
        row = int(input("Enter the row: "))
        column = int(input("Enter the column: "))
        cell = self.__GetCell(row, column)
        symbol = cell.GetSymbol()
        if symbol == "-" or symbol == "@" or not(cell.CheckSymbolAllowed(symbol)):
          print("Cannot remove the symbol in this cell.")
        else:
